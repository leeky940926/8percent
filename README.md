...
1억건의 거래가 생성되면 데이터 중간에 빌 수도 있고,

최소 5년~최대 10년동안 고객의 거래내역을 저장하고 삭제할 수 있는 규정 상 테이블 하나를 DROP TABLE 해줄 수도 있습니다.
...
파티션이란 데이터베이스를 여러 부분으로 분할하는 것입니다.

물리적인 데이터 분할이 있지만 DB에 접근하는 app의 입장에서는 이를 인식하지 못합니다.

데이터가 너무 커져서 조회하는 시간이 길어지는 것을 최소화 시키기 위해서 사용합니다.

파티션을 이용하여 데이터베이스 설계를 하려고 하였지만, sqlite가 파티션을 지원을 하지 않아 수동으로 파티션을 해주었습니다.

거래 내역 테이블을 2020년과 2021년으로 나누어서 한 테이블에 몰리는 데이터의 과부화가 줄어들도록 데이터베이스를 설계하였습니다.

3. index = True

쿼리가 실행되었을 때, 저장공간 접근 횟수를 최소화 해 데이터베이스의 성능을 최적화 하기위해,
또 데이터를 빠르게 찾고 또 접근하기 위해 created_at에 대하여 indexing을 해주었습니다.
